...
[[ -z "$BASH_LINENO" || BASH_LINENO[0] -eq 0 ]] || (( ! SOURCE_GHWF_PRINT_ERROR_SH )) || return 0 || exit 0 # exit to avoid continue if the return can not be called
...
function gh_print_error_ln()
{
  ...
  if [[ -n "${GHWF_PRINT_ERROR_BUF_STR+x}" ]]; then
    ...
    # with check on integer value
    if [[ -n "$PRINT_ERROR_LAG_FSEC" && -z "${PRINT_ERROR_LAG_FSEC//[0-9]/}" ]] && (( PRINT_ERROR_LAG_FSEC > 0 )); then
      sleep $PRINT_ERROR_LAG_FSEC
    fi
